...
def extract_project_details(text, MASTER_SKILLS):
    ...
    if not project_section_match:
        ...
        sections = ['education', 'skills', 'projects', 'certifications', 'awards', 'publications', 'interests', 'hobbies']
        end_index = len(text)
        for section in sections:
...
"""
    ...
    <html>
        <body>
            ...
            <p>Congratulations on successfully clearing the ScreenerPro resume screening process with an impressive score of <strong>{score:.1f}%</strong>!</p>
...
def resume_screener_page():
    ...
    if 'comprehensive_df' not in st.session_state:
        st.session_state['comprehensive_df'] = pd.DataFrame(columns=[
            ...
            "Semantic Similarity", "Resume Raw Text",
    ...
    if jd_text and resume_files_to_process:
        # Start overall timer
        ...
        for file in resume_files_to_process:
        ...
        with st.spinner(f"Extracting text from {total_resumes} resumes..."): # Spinner instead of info message
        ...
        with st.spinner(f"Generating embeddings for {len(successfully_extracted_texts_map)} resumes and JD..."): # Spinner instead of info message
        ...
        with st.spinner(f"Analyzing {len(successfully_extracted_texts_map)} resumes with AI models concurrently..."): # Spinner instead of info message
            processing_args = []
        ...
        st.markdown("---")
        # Removed the Candidate Score Comparison Chart as requested.
        ...
        # dark_mode = st.session_state.get("dark_mode_main", False) # This line is now removed
        ...
        if not st.session_state['comprehensive_df'].empty:
            ...
            # Parse the JSON string back to a dictionary for display
            matched_kws_categorized_str = top_candidate['Matched Keywords (Categorized)']
        ...
        if not filtered_display_df.empty and 'CGPA (4.0 Scale)' in filtered_display_df.columns:
            if not (min_cgpa_filter == 0.0 and max_cgpa_filter == 4.0):
                filtered_display_df = filtered_display_df[
                    ((filtered_display_df['CGPA (4.0 Scale)'].notnull()) | (filtered_display_df['CGPA (4.0 Scale)'].isnull() & min_cgpa_filter == 0.0)) &
                    (filtered_display_df['CGPA (4.0 Scale)'].fillna(0) >= min_cgpa_filter) & # Fill NA with 0 for comparison
                    (filtered_display_df['CGPA (4.0 Scale)'].fillna(4.0) <= max_cgpa_filter)  # Fill NA with 4.0 for comparison
                ]
                    ...
                    if candidate_data_for_cert.get('Certificate Rank') != "Not Applicable":
                        ...
                        with col_cert_download:
                            if certificate_pdf_content:
                                ...
                                # Send email only after the download button is interacted with (or PDF is generated)
